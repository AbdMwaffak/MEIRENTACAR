...
// import { selector } from '../redux/Actions/SelectorActions';
import './flter.css';
...
function Flter() {
  ...
  return (
    <div className='ff66'>
      <span onClick={flButtonnHandler} className='flButtonn' style={{display : window.location.pathname === '/' || window.location.pathname.includes('search')  || window.location.pathname.includes('cars') ? 'flex' : 'none'}}>
          Filter 
      ...
      <div className={flButtonn ? 'sid-active' : 'sid-unactive'}>
        <div className='fl'>
          ...
          <button className='closeB'  onClick={flButtonnHandler}> X </button>
          </div>
          <Form className='mainMenue'>
            <div className='menue'>
              <Form.Label className='lab'> Sort </Form.Label>
              <Form.Select className="sortMenue"
                defaultValue="all"
                onChange={(e) => setSort(e.target.value)}>
                <option className='sortoption'> non sort </option>
                <option className='sortoption' value='price'> by price h - l </option>
                <option className='sortoption' value='popularity'> by popularity </option>
                <option className='sortLi' value='createdAt'> by latest </option>
                {/* <option className='sortLi'> all </option> */}
              </Form.Select>
            </div>
            <div className='menue'>
              <Form.Label className='lab'> Seats </Form.Label>
              <Form.Control className="sortMenue"
                max={20}
                min={1}
                type='number'
                value={seats}
            ...
            <div className='menue'>
              ...
              <Form.Select className='brandMenue'
                defaultValue="all"
                onChange={(e) => setBrand(e.target.value)}>
                {Brands.map((brand, index) => (
                  <option className='brandLi' key={index}> {brand} </option>
                ))}
              </Form.Select>
            </div>
            ...
            <div className='menue'>
              ...
              <Form.Select className='brandMenue'
                ...
                {Colors.map((color, index) => (
                  <option className='brandLi' key={index}> {color} </option>
                ))}
              </Form.Select>
            </div>
            ...
            <div className='menue'>
              ...
              <Form.Select className='typeMenue'
                // defaultValue="all"

                onChange={(e) => setCategory(e.target.value)}>
                <option>   </option>
                {Categorys.map((cate, index) => (
                  <option className='brandLi' key={index}> {cate} </option>
                ))}
              </Form.Select>
            </div>
            <div className='babutton'>
              <div type='submit' className='fsButtonn'
